...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_post($url,$param,$post_file=false){
		...
		if (is_string($param) || $post_file) {
			//$strPOST =['media' => new CURLFile($param),];
			$strPOST = $param;
		} else {
			$aPOST = array();
	...
	public function getCache($appid='',$appsecret='')
	{
		if (!$appid || !$appsecret) {
			$appid = $this->appid;
	...
	public function createMenu($data){
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				return '错误提示'.$this->errMsg.'错误码'.$this->errCode;
			}
			return '提示'.$json['errmsg'].'错误码'.$json['errcode'];
		}
		return false;
	}
